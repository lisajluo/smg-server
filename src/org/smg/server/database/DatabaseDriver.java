...
public class DatabaseDriver {
	...
	/**
	 * Returns a list of entities given a query by kind (ex: DEVELOPER),
	 * property (ex: EMAIL), and the string query (ex: "foo@bar.com").
	 */
	...
	public static void insertMatchEntity(JSONObject match) throws JSONException {
		...
		entity.setProperty("playerThatHasTurn",
				match.getInt("playerThatHasTurn"));
		entity.setUnindexedProperty("gameOverScores",
				match.getJSONObject("gameOverScores").toString());
		entity.setProperty("gameOverReason", match.getString("gameOverReason"));
		entity.setUnindexedProperty("history", match.getJSONArray("history")
				.toString());
	...
	private static String updatePlayer(Player player) {
		...
		try {
			...
			String asLocal = playerInfo
					.containsKey(PlayerProperty.ACCESSSIGNATURE.toString()) ? playerInfo
					.get(PlayerProperty.ACCESSSIGNATURE.toString()) : "";
	...
	/**
	 * login a player by id and password If login success, update access
	 * signature and return If login fail, throw EntityNotFoundException for
	 * WRONG_PLAYER_ID
	 * 
	 * @param playerId
